...
def main():
    ...
    # Consume one CobbleStone page (<=100 requests) at a time and persist it
    # immediately — listing all ~197 pages before the first upsert (the
    # original design) meant an interruption during that ~25min listing phase
    # threw away all of it. Per-request commits still bound crash loss to at
    # most one in-flight request within a page.
    for page in iter_request_pages(token, start_after_id=start_after_id, limit=args.limit):
        for request in page:
